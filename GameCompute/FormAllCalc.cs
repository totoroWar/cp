...
{
    public partial class FormAllCalc : Form
    {
        Thread thread = null;
        GameServices.IGame game = new GameServices.Game();
        OtherObject Go = null;
        public FormAllCalc(OtherObject go)
        {
            Go = go;
            InitializeComponent();
            Shown += FormCalc_Shown;
            Text = Go.GameClassName;
        }
        ...
        public void DoIt()
        {
            ...
            if (1 == processMessage.Code)
            {
                ...
                {
                    FileStream fs = new FileStream(System.Environment.CurrentDirectory + "\\Log\\" + (1 == processMessage.Code ? "S_" + Go.GameClassID + "_" : "E_" + Go.GameClassID + "_") + DateTime.Now.ToString("yyyyMMddHH") + ".log", FileMode.Append);
