...
namespace GameUI.Controllers
{
    ...
    public class UIController : BaseController
    {
        ...
        public UIController(IGame game, IMenu menu, IFinance finance, ISystem system, IUser user)
        {
            serGame = game;
            serMenu = menu;
            serFinance = finance;
        ...
        public ActionResult UCenter(int? pageIndex)
        {
            ...
            else if ("dataChange" == MethodType)
            {
                ...
                pageURL = Url.Action("UCenter", "UI", new { method = MethodType, dts = dts, dte = dte, dcttype = dcttype });
        ...
        public ActionResult Bank(int? pageIndex)
        {
            ...
            if ("crhl" == MethodType)
            {
                ...
                string pageURL = Url.Action("Bank", "UI", new { method = MethodType });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
            ...
            else if ("ichl" == MethodType)
            {
                ...
                string pageURL = Url.Action("Bank", "UI", new { method = MethodType, dts = _dts, dte = _dte, type = _type });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
            }
        ...
        private ActionResult PrivateMyInfo()
        {
            ...
            if (null != aguData)
            {
                ...
                ajax.Code = 1;
                ajax.Data = new { M = aguData.wgs014.uf001.ToString("N2"), P = aguData.wgs014.uf004.ToString("N2"), L = aguData.u015, LN = serUser.GetUserLevel(true)[aguData.u015], MSG = serSystem.GetUnReadMessage(0, UILoginUser.u001, 0), POS = postName, CBC = combuyCount };
            }
        ...
        private ActionResult PShowList()
        {
            ...
            if (Session["UILoginID"] != null)
            {
                ajax.Data = new GameServices.User_ORZ().Get053List();
            }
            else
            {
                ajax.Data = "";
        ...
        private ActionResult AllCancelOrder(string flag, string orderID, ref string xmsg)
        {
            if ("cancelorder" == flag)
            {
                ...
                {
                    xmsg = "订单编号为空0x2";
                    return Json(new { Message = "empty", stats = "error", data = "没有符合撤单条件的订单" });
                }
            }
        ...
        public ActionResult Record(int? pageIndex)
        {
            ...
            if ("orderDefault" == MethodType)
            {
                ...
                ViewData["GameList"] = serGame.GetGameListByCache();
                ViewData["GameMethodList"] = serGame.GetGameMethodListByCache();
                pageURL = Url.Action("Record", "UI", new { method = MethodType, orderDTS = ViewData["DTS"], orderDTE = ViewData["DTE"], orderUser = orderUserType, orderCancel = orderCancel, orderMethod = orderGameMethod, orderGame = orderGame, orderGameClass = orderGameClass, orderID = orderID, orderIsWin=_orderIsWin });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
            }
            #endregion
            #region 追号
            else if ("orderTrace" == MethodType)
            {
                ...
                pageURL = Url.Action("Record", "UI", new { method = MethodType, orderDTS = ViewData["DTS"], orderDTE = ViewData["DTE"], orderUser = orderUserType, orderStatus = orderStatus, orderMethod = orderGameMethod, orderGame = orderGame, orderGameClass = orderGameClass, orderID = orderID, orderUserName = orderUserName });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
            ...
            else if ("orderTraceDetail" == MethodType)
            {
                ...
                pageURL = Url.Action("Record", "UI", new { method = MethodType, key = _traceKey });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
        ...
        public ActionResult Combine(int? pageIndex)
        {
            ...
            ViewData["CombuyList"] = combuyList;
            var pageURL = Url.Action("Combine", "UI", new { method = MethodType, dts = ViewData["DTS"], dte = ViewData["DTE"], orderGame = orderGame, issue = issue, status=status });
            ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
        ...
        public ActionResult Member(int? pageIndex)
        {
            ...
            if ("accountList" == MethodType)
            {
                ...
                pageURL = Url.Action("Member", "UI", new { method = MethodType, parentID = parentID, userName = userName, userStatus = status, regDTS = rDTS, regDTE = rDTE, loginDTS = lDTS, loginDTE = lDTE, amountType = amtT, amountTypeV = amtV, pointType = pntT, pointTypeV = pntV });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
            }
        ...
        public ActionResult Report(int? pageIndex)
        {
            ...
            else if ("dataChange" == MethodType)
            {
                ...
                var recordCount = 0;
                var dataChageList = serFinance.GetDataChangeList(UILoginUser.u001, dcttype, 0, string.Empty, dts, dte, PageSize, (int)pageIndex, out recordCount);
                pageURL = Url.Action(ActionName, "UI", new { method = MethodType, dts = dts, dte = dte, dcttype = dcttype });
                ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
                ViewData["DataChangeList"] = dataChageList;
            }
            else if ("stockDetails" == MethodType)
            {
                ...
                //pageURL = Url.Action(ActionName, "UI", new { method = MethodType });
                //ViewData["PageList"] = _NWC.PageList.GetPageList(PageSize, recordCount, (int)pageIndex, PageBloclSize, "page_list_block", "pageIndex", pageURL);
                ...
                int recordCount = 0;
                xlist = ua.GetFHLogForUser((int)pageIndex, PageSize, out recordCount, UILoginUser.u001);
                ...
                foreach (DBModel.wgs056 item in xlist)
                {
                    ...
                    if (memoney > 0)
                    {
                        if (item.s005 == 1)
                        {
                            mytr += "<td class = 'fc-yellow'>" + memoney.ToString("N4") + "</td>";
                        }
                        else
                        {
                            mytr += "<td>" + memoney.ToString("N4") + "</td>";
                        }
                    }
                    else
                    {
                    ...
                    mytr += "<td>" + (item.s002 * -1).ToString() + "</td>";
                ...
                foot += "<tr>";
                foot += "<td>";
                foot += ua.GetSum(UILoginUser.u001);
                foot += "</td>";
                foot += "<td>-</td>";
                foot += "<td>-</td>";
                foot += "<td>-</td>";
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public ActionResult Play(int? gameID, int? gameClassID)
        {
        ...
        public ActionResult Message(int? pageIndex)
        {
            ...
            var pageURL = Url.Action("Message", "UI");
