                ...
                <tr class="tr_ct">
                    ...
                    <td>
                        <%foreach (var cp in cpl)
                          { %>
                        <label style="padding: 5px;" for="l_<%:cp.ct001 %>" title="<%:cp.ct002 %>-<%:cp.ct003 %>-<%:cp.ct004 %>"><%:cp.ct002 %><input id="l_<%:cp.ct001 %>" type="checkbox" name="u017" value="<%:cp.ct001 %>" /></label>
                        <%} %>
                    </td>
                </tr>
                        ...
                        <table class="table-pro" width="100%">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    %>
                                    <input type="hidden" name="[<%:gpIndex %>].up001" value="0" class="input-text w50px" />
                        ...
                        $.ajax({
                            ...
                            success: function (a, b)
                            {
                                ...
                                for (var i = 0; i < a.Data.length; i++)
                                {
                                    var item = a.Data[i];
                                    var row = "<tr>";
                                    row += "<td class='title'>" + item.GameClassName + "-" + item.ShowPointName + "<input type='hidden' name='[" + i + "].gp001' value='" + item.GameClassPrizeID + "' /></td>";
                                    row += "<td>" + "系统定位返点" + item.SystemPoint + "%,不定位返点"+item.SystemPointX+"<br />可分配定位返点" + item.CurrentHavePoint + "%不定位返点" + item.CurrentHavePointX + "<br />赚下级定位返点<input name='[" + i + "].up002' type='text' class='input-text w50px' value='0.0000'>%，不定位返点<input name='[" + i + "].up004' type='text' class='input-text w50px' value='0.0000'>%<br />" + "</td>";
                                    row += "</tr>";
                                    $("#pd_info").append(row);
                                }
                            },
