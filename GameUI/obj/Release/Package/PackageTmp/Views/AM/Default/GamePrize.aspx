    ...
    <table width="100%" class="table-pro table-color-row">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="link-tools"><a class="dom-hidex" href="javascript:window.parent.ui_show_tab('<%:gameClassList.Where(exp=>exp.gc001==item.gc001).FirstOrDefault().gc003 %>-<%:item.gp003 %>－显示玩法','/AM/GameClassPrize?method=config&gameClassID=<%:item.gc001 %>&gameClassPrizeID=<%:item.gp001 %>',true);" title="显示玩法">显示玩法</a><a class="dom-hidex" href="javascript:window.parent.ui_show_tab('<%:item.gp003 %>奖金配置','/AM/GamePrizeData?method=config&gameClassID=<%:item.gc001 %>&gameClassPrizeID=<%:item.gp001 %>',true);" title="奖金配置">奖金配置</a></td>
            </tr>
            <%
                      listIndex++;
                  }/*foreach*/
