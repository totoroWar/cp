...
<div class="xtool">
    <form action="/AM/AllUser" method="get">
        ...
        <input type="button" id="btn_set_status" value="设置" />
        <input type="button" id="a-table-select" value="全选" />
        <input type="button" id="a-table-clear-select" value="取消" />
</form>
</div>
    ...
    <div id="edit_user_dlg">
        ...
        <table class="table-pro g_nco tp5" width="100%">
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="input-text w50px" id="red_percent" name="red_percent" readonly="readonly" />%<font color="red"> （若要修改分红比例，请到"账户->分红设置"中更改）</font>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    $("a[name='delete_user']").click(function () {
        if (confirm("删除用户将删除所有相关信息！可能会导致账目不对")) {
            var key = $(this).attr("data");
            $.ajax({
                async: false, timeout: _global_ajax_timeout, type: "POST", cache: false, url: "/AM/AgentList?method=deleteUser", data: { key: key },
                success: function (a, b) {
                    _check_auth(a);
                    var _robj = eval(a);
                    if (1 == _robj.Code) {
    ...
    $("#btn_set_status").click(function ()
    {
        ...
        if (confirm("确认将选中记录设置状态？")) {
            var form_data = $("#form_edit_user").serialize();
            $.ajax({
                async: false, timeout: _global_ajax_timeout, type: "POST", cache: false, url: "/AM/AllUser?method=setallstatus", data: { __RequestVerificationToken: $("input[name='__RequestVerificationToken']").val(), key: key, method: "setallstatus",status:status },
                success: function (a, b) {
                    _check_auth(a);
                    var _robj = eval(a);
                    if (0 == _robj.Code) {
