...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">切换线路</asp:Content>
                ...
                <%foreach (var item in list)
                  {
                      ...
                      %>
                <li><a target="_parent" class="ui-post-loading" style="color:green; padding:5px;" href="http://<%:item.Key %>/UI/ChangeLine?SSOKey=<%:ViewData["Key"] %>&Account=<%:ViewData["Account"] %>"><%:item.Key %></a>（在线率<%=percent.ToString("N4")  %>）</li>
                <%} %>
            </ul>
        </div>
    </div>
