(function ($) {
    ...
    $.playInit = function (opts) {
		...
		function () {
		    ...
		    $pid = $(this).html();
		    $.blockUI({
		        message: '<div style="width:200px;padding:10px 100px;background-color:#fff;border:4px #666 solid;"><img src="' + pri_imgserver + '/images/comm/loading.gif" style="margin-right:10px;">正在读取投注详情...</div>',
		        overlayCSS: {
		                ...
		                if (data.stats == "error") {
		                    ...
		                    $("#cancelproject").click(function () {
		                        if (confirm("真的要撤单吗?" + (data.need == 1 ? "撤销此单将收取撤单手续费金额:" + data.money + "元." : ""))) {
		                            $.blockUI({
		                            message: '<div style="width:200px;padding:10px 100px;background-color:#fff;border:4px #666 solid;"><img src="' + pri_imgserver + '/images/comm/loading.gif" style="margin-right:10px;">正在提交撤单请求...</div>',
		                            overlayCSS: {
		                            backgroundColor: "#000000",
		                            opacity: 0.3,
		                            cursor: "wait"
    ...
    var lt_smalllabel = function (opts) {
        ...
        $("span[id^='smalllabel_']", $($.lt_id_data.id_smalllabel)).click(function () {
            ...
            $(this).data("ischecked", "yes").attr("class", "tab-front").parent().attr("class", "act");
    ...
    $.fn.lt_selectarea = function (opts) {
        ...
        function selectNum(obj, isButton) {
            ...
            if ($.lt_dantuo) {
                if (data_sel[0].length + 1 >= $.lt_danlen && place == 0) {
                    ...
                    $.each($('div[name="lt_place_0"][class="on"]'),
					function (i, n) {
					    if ($(this).html() == lastnumber) {
					        $(this).attr("class", "");
					        data_sel[0] = $.grep(data_sel[0],
							function (n, i) {
							    return n == lastnumber
							},
							true)
                ...
                if (data_sel[unplace].contains(number)) {
                    $.each($('div[name="lt_place_' + unplace + '"][class="on"]'),
					function (i, n) {
					    if ($(this).html() == number) {
					        $(this).attr("class", "");
					        data_sel[unplace] = $.grep(data_sel[unplace],
							function (n, i) {
							    return n == number
							},
							true)
        ...
        $($.lt_id_data.id_sel_insert).unbind("click").click(function () {
            ...
            var cfhtml = '<tr style="cursor:pointer;"><td class="tl_li_l" width="4"></td><td>' + noshtml.substring(0, 20) + "</td><td width=25>" + $.lt_method_data.modes[modes].name + '</td><td width=80 class="r">' + nums + lot_lang.dec_s1 + '</td><td width=80 class="r">' + times + lot_lang.dec_s2 + '</td><td width=120 class="r">' + money + lot_lang.dec_s3 + '</td><td class="c tl_li_r" width="16"><input type="hidden" name="lt_project[]" value="' + serverdata + '" /></td></tr>';
            var $cfhtml = $(cfhtml);
            if ($.lt_total_nums == 0) {
                $($.lt_id_data.id_cf_content).children().empty()
            }
        ...
        $.ajax({
            ...
            success: function (data)
            {
				...
				function (i, n) {
				    ...
				    $shtml += "<td>" + n.issue + "</td>";
				    $shtml += "<td class='" + n.code+ "'>" + n.codeShort + "</td>";
